/* 
Edits by Emre Gonultas, eg566@cornell.edu
...
function bibtex_js_draw() {
    ...
    $.when.apply($, requests).then(function() {
               ...
               //bibstring.replace(/\w*(?<!=\s*)\{+.*\}+/g, '');

        
        bibstring=bibstring.replace(/\w*(?<!=\s*)\{+\b[A-Z]+\b\}+/g,function(match) {
        ...
        monthstrings = {
            ...
            abbrv=''
            var request = $.ajax({
                url: 'IEEEabrv.bib',//$('bibtex').data('url'),
                async: false,
                dataType: "text"
                })
                .done((data) => abbrv += data)
                .fail((request, status, error) => console.error(error));
        bibstring=bibstring.replace(/month\s*=\s*\w*,/ig,function(match) {
            if (monthstrings[String(match.slice(-4,-1))]){
                return 'Month='+monthstrings[String(match.slice(-4,-1))]+',';
            }
            else{
                return 'Month= {'+ match.slice(1,-1)+'},';
            }
        });
        bibstring=bibstring.replace(/journal.*=.*,/ig,function(match1) {
            match_sorg=match1.split("=")[1].slice(0,-1)
            match_s=match1.split("=")[1].slice(0,-1).replace(/\s/g, '')
            if (match_s[1]==='{')
            {
                return match_s.slice(1,-1);
            }
            else
            {
                var abr2=String(abbrv).match(new RegExp('\@STRING\{'.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')+match_s+ ' [^a-zA-Z]*=.*"\}'.replace(/[.*+?^${}()|[]\]\\]/g, '\\$&'),'ig'));
                ////old original var abr2=abbrv.match(new RegExp('\@STRING\{'+ match_s.replace(/\s/g, '') +'[^a-zA-Z]*=.*"\}','ig'));
                //console.log(match_s);
                //console.log('\\@STRING\\{'+ match_s.replace(/\s/g, '') +'[^a-zA-Z]*=.*\"\\}')
                //console.log(abr2);
                if (abr2){
                abr3=String(abr2).replace(/\w*(?<!=\s*)\{+\b[A-Z]+\b\}+/g,function(match) {
                  return match.slice(1,-1);
                });
                return 'Journal= {'+String(abr3).split('"')[1]+'},';//+abr2.split("=")[1];
                }
                else{
                    return 'Journal= {'+match_sorg+'},';//+abr2.split("=")[1];
                }
            }
        });
        bibstring=bibstring.replace(/booktitle.*=.*,/ig,function(match1) {
            match_s=match1.split("=")[1].slice(0,-1).replace(/\s/g, '')
            if (match_s[1]==='{')
            {
                return match_s.slice(1,-1);
            ...
            else
            {
                ...
                return 'Booktitle= {'+String(abr3).split('"')[1]+'},';//+abr2.split("=")[1];
            }
        });
        bibstring=bibstring.replace(/publisher.*=.*,/ig,function(match1) {
            //match_s=match1.split("=")[1].slice(0,-1).replace(/\s/g, '')
            //console.log(match_s)
            //if (match_s==='academic')
            //{
        ...
        //function(v) { return v.substring(1,v.length-1); }
